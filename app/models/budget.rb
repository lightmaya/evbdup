...
class Budget < ActiveRecord::Base
  ...
  after_save do
    # 每次保存后 更新对应的订单、网上竞价、协议议价的预算金额
    [Order, BidProject, Bargain].each do |cls_name|
      obj = cls_name.find_by(budget_id: self.id)
      ha = cls_name == Bargain ? { total: self.total } : { budget_money: self.total }
      if obj.present?
        obj.update(ha)
        break
      end
