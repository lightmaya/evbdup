...
# class Kobe::BudgetsController < KobeController
  ...
  # def new
  #   @budget = Budget.new
  #   @myform = SingleForm.new(Budget.xml, @budget, { form_id: "budget_form", upload_files: true, min_number_of_files: 1, action: kobe_budgets_path, title: "<i class='fa fa-pencil-square-o'></i> 新增预算" })
  # end

  # def create
  #   budget = create_and_write_logs(Budget, Budget.xml, {}, {name: "#{current_user.real_department.name} #{Time.now.to_date} 预算", department_id: current_user.department.id, dep_code: current_user.real_dep_code })
  ...
  # def edit
  #   @budget = Budget.find_by id: params[:id]
  #   @myform = SingleForm.new(Budget.xml, @budget, { form_id: "budget_form", upload_files: true, min_number_of_files: 1, action: kobe_budget_path(@budget), method: "patch", title: "<i class='fa fa-pencil-square-o'></i> 修改预算" })
  # end
