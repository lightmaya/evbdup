class Kobe::AssetProjectsController < KobeController
  ...
  def new
    ...
    @asset_project.name = '车辆费用报销'
    slave_objs = [@asset_project.items.build]
    @myform = MasterSlaveForm.new(AssetProject.xml,AssetProjectItem.xml,@asset_project,slave_objs,{form_id: 'new_asset_project', upload_files: true ,  title: "<i class='fa fa-pencil-square-o'></i> 新增车辆费用报销", action: kobe_asset_projects_path, show_total: true, grid: 3},{title: '费用明细', grid: 3})
  end
  ...
  def update
    update_msform_and_write_logs(@asset_project, AssetProject.xml, AssetProjectItem, AssetProjectItem.xml, {:action => "修改费用信息", :slave_title => "费用信息"}, {name: create_name})
    redirect_to kobe_asset_projects_path 
  end

  def edit
    slave_objs = @asset_project.items.blank? ? [@asset_project.items.build] : @asset_project.items
    @myform = MasterSlaveForm.new(AssetProject.xml,AssetProjectItem.xml , @asset_project, slave_objs,{form_id: 'new_asset_project', title: "<i class='fa fa-wrench'></i> 修改日常费用" , action: kobe_asset_project_path(@asset_project), method: "patch", show_total: true, grid: 3},{title: '费用明细', grid: 3})
  end
  ...
  private
   ...
   #是否有权限操作项目
    ...
    def create_name
      "#{params[:asset_projects][:deliver_at]} #{params[:asset_project_items][:asset_name].values.uniq.join(',')}"
    end
